...
관계형 데이터베이스 > 절차형 언어가 아님 > 제어문이 없음 > 모든 문장이 독립적
...
2. 문자형
    ...
    c. varchar2 -- 대부분 많이 사용 (이전에는 성능 차이가 있었지만 이젠 없음)
...
3. 날짜/시간형 > data 사용
    a. date
        - 년월일시분초
        - 기원전 4712년 1월 1일 ~ 9999년 12월 31일까지 표현 가능
...
create table tblType (
    --num number
    --num number(3) -- -999 ~ 999 정수(세자리)
    --num number(4,2) --앞 : 자리수(전체 자릿수) / 뒤 : 소수 이하 자릿수 -- -99,99 ~ 99.99
    ...
    --txt nchar(10) --바이트가 아닌 문자수 == txt char(10 char)
);
...
insert into tblType (num) values (3.99); --실수 리터럴 > 반올림 됨 4가 저장
...
insert into tblType (txt2) values ('홍길동');
