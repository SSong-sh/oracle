...
create table tblScore(
    seq number not null,                 
    pointsAllocatedNum number not null,  
    attendanceScore number not null,           
    handWritingScore number not null,    
...
create table tblReview(                     -- 占쎈즲占쎄퐣 뵳 됰윮 占쎈 믭옙 뵠 뇡占 
    ...
    studentNum number,                     -- 占쎈땾 뙴 슣源  甕곕뜇 깈
...
create table tblReviewList(
    seq number,
    bookNum number,
    reviewNum number
);
alter table tblReviewList
    add constraint tblreviewlist_seq_pk primary key(seq);
...
alter table tblReview
    add constraint tblreview_graduatenum_fk foreign key(studentNum) references tblGraduate(seq);
    
alter table tblReviewList
    add constraint tblReviewList_bookNum_fk foreign key(bookNum) references tblBook(seq);
alter table tblReviewList
    add constraint tblReviewList_reviewNum_fk foreign key(reviewNum) references tblReview(seq);
