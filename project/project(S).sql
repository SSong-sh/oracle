...
create table tblOpenCurriculum (             -- 揶쏆뮇苑뺞�⑥눘�젟 占쎈�믭옙�뵠�뇡占�
    ...
    name varchar2(100),                      -- 揶쏆뮇苑뺞�⑥눘�젟 占쎌뵠�뵳占�
...
create table tblVolunteer (                 -- 筌욑옙占쎌뜚占쎌쁽 占쎈�믭옙�뵠�뇡占�
    ...
    ssn number,                             -- 雅뚯눖占쎌눖踰묉에�빖苡뀐옙�깈
    tel number                              -- 占쎌읈占쎌넅甕곕뜇�깈
);
...
create table tblLectureSchedule(
    ...
    seqEmployee number not null,             --吏곸썝踰덊샇(FK)
...
create table tblTeacherMajor(
    ...
    seqSubject number not null,  --怨쇰ぉ踰덊샇(FK)
    seqEmployee number not null  --吏곸썝踰덊샇(FK)
);
...
create table tblPointsAllocated(
    ...
    seqOpenSubjectInfo number not null,         --媛쒖꽕怨쇰ぉ踰덊샇(FK)
    seqEmployee number not null,                --吏곸썝踰덊샇(FK)
...
create table tblScore(
    ...
    seqPointsAllocated number not null,  --諛곗젏踰덊샇(FK)
