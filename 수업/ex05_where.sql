...
--1. 몸무게가 60kg이상이고, 키가 170cm 미만인 사람을 가져오세요.
...
--tblInsa
select * from tblinsa;
--3. 부서가 '개발부'이고, 급여가 150만원 이상 받는 직원을 가져오세요.
...
--4. 급여 + 수당을 합한 금액이 200만원 이상 받는 직원을 가져오세요.
select 
    *
from tblinsa
...
/*
    ...
    1. 접속 
        - 호스트명: 접속할 오라클 서버의 IP 주소(or 도메인명) > "localhost" (내컴퓨터)
        - 포트번호 : "1521"
        - SID : "xe"
        - 사용자명 : "hr"
        - 암호 : "java1234"
...
--2. 문자형(문자코드)
select * from tblInsa where name >= '이순신'; (이씨 이하인 사람들)
...
--입사 날짜는 date형, 문자열과 비교 => 자동형변환 시켜줌 > 직접 비교가 가능 
desc tblInsa; 
...
select name from tblInsa where name like '_수'; -- 글자수를 맞춰줘야함

select * from employees where first_name like 'S_____'; 
select * from employees where first_name like 'S____';

select name from tblInsa where name like '김%'; -- 김으로 시작하는 사람
select name from tblInsa where name like '%동'; -- 끝이 동으로 끝나는 사람
select name from tblInsa where name like '%수%'; -- 수가 포함된 모든 사람
