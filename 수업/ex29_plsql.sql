/*
    ...
    프로시저, Procedure
    - 메서드, 함수 등..(자바 수업 때 모두 같은 말이라고 이야기 함)
    - 순서가 있는 명령어들의 집합
...
set serveroutput on; -- 밑에 기본 설정이 화면에 안 보이게 되어 있어서 on을 통해서 설정 바꾸기 -- 현재 세선에서만 유효(매번 켜야됨)
set serverout on;
...
/ -- pl/sql에서 슬래시를 안 적으면 블럭을 잡지 않고 자동 실행이 불가능하기 때문에 블럭마다 슬래시를 작성해준다.
...
begin
    ...
    insert into tblTODO
        values ((select max(seq) + 1 from tblTODO), '할 일', sysdate, null);
...
desc tblInsa; -- 원본 테이블을 참조할 때는 길이를 맞춰서 작성해야한다.
...
declare
    --vbuseo varchar2(15);
    vbuseo tblInsa.buseo%type; -- 알아서 타입 참조
begin
...
begin
    --1. select name, age, height, weight, couple into row
...
alter table tblGugudan
    add constraint tblgugudan_dan_num_pk primary key(dan, num); -- 이걸 추천
...
declare
    ...
    cursor vcursor is select name from tblInsa; -- 정의O, 실행X
...
begin
    ...
    open vcursor; -- 커서 열기 > select 실행 > 결과셋을 커서가 참조
        ...
        loop
            fetch vcursor into vname;
            exit when vcursor%notfound; -- 데이터가 없으면 true
            
            dbms_output.put_line(vname);
            
--            if vcursor%notfoun then
...
begin
     ...
     loop
        ...
        dbms_output.put_line(vn || ',' || vjikwi || ',' || vbasicpay);
...
begin
    ...
    --open vcursor;
    for vrow in vcursor loop -- 이렇게 for문에 loop를 적으면 코드가 많이 줄어든다.
...
begin
    vnum := 100;
    dbms_output.put_line(vnum); -- 메서드를 정의한 거라서 출력되지 않는다.
end;
/
...
-- 프로시저 호출
begin
    procTest; -- 이건 plsql건데 plsql은 begin~end에서만 실행할 수 있다. > ANSI-SQL 즉 표준 sql이 아니라서 그렇다.
end;
/
...
-- 1. 매개변수가 있는 프로시저
create or replace procedure procTest(pnum number) -- 매개 변수
is
    vnum number; -- 일반 변수
begin
    vnum := pnum * 2;
    dbms_output.put_line(vnum);
end procTest; -- 가독성을 높이기 위해서 작성하는 것이다. > 주석의 개념이 아니라 오타나면 안 된다.
/
...
create or replace procedure procTest( -- 매개변수라서 소괄호에서 콤마로 구분하는 건데 모양 때문에 문장마다 ;을 작성하는 구분이랑 헷갈릴 수 있다.
    pwidth number, 
    pheight number 
)
is
...
create or replace procedure procTest (
    pwidth number,
    pheight number default 10 -- default는 끝에서부터 채워나가는 것만 가능하다. 
)
is
    varea number;
begin
...
create or replace procedure procTest (
    pnum1 number,           -- in number
    pnum2 in number,
    presult out number,     -- out parameter // 반환값 역할
    presult2 out number,     -- out parameter // 반환값 역할
    presult3 out number     -- out parameter // 반환값 역할
)
is
begin
...
declare
    ...
    vtemp3 number;
begin
    procTest(10, 20, vtemp, vtemp2, vtemp3); -- 식은 피할당자로 사용될 수 없습니다. // 아무것도 없는 변수를 넘겨줘서 주소가 넘어간다.
...
/*
    ...
    2. procTest2
        - 직원 번호 전달 > in
        - 같은 지역에 사는 직원수? 같은 직위의 직원수? 해당 직원보다 급여를 더 많이 받는 사람수?

*/

create or replace procedure procTest1 (
    pbuseo in varchar2, -- 길이를 선언할 수 없다.
    pnum out number
)
is
begin
...
create or replace function fnSum(
    pnum1 in number,
    pnum2 in number
    --presult out number //아웃 파라미터를 적지 않는다.
) return number
is
begin
...
begin
    ...
    vresult := fnSum(30, 40); -- PL/SQL에서 잘 사용 안 한다. > ANSI-SQL에서 사용한다.
    dbms_output.put_line(vresult);
...
select
    name, buseo, jikwi, fnGender(ssn) as gender -- 저장 프로시저는 사용하기 어렵고 function은 return 값이 유일하기 때문에 사용하기 편한다.
from tblInsa;
...
/*
    ...
    - 특정 테이블 지정 > 지정 테이블을 오라클 감시 > 사건 발생(insert / update / delete) > 미리 준비한 프로시저 호출
...
-- 24시간 감시하기 때문에 비용이 많이 들어간다.
-- tblInsa > 직원 삭제 // create 전에 delete하지 않을 건 rollback하고 진행하기
create or replace trigger trgInsa -- 트리거는 만들자마자 바로 실행됨
    before      -- 삭제가 발생하기 !직전!에 구현부를 실행해라
    delete      -- 삭제가 발생하는지 검사?
...
begin
    ...
    if to_char(sysdate, 'dy') = '목' then
        ...
        -- : throw new Exception()
        -- : -20000 ~ -29999
        raise_application_error(-20001, '목요일에는 퇴사가 불가능합니다.'); --20000은 사용 중이다.
    end if;
...
delete from tblInsa where num = '1006'; -- 삭제 되지 않는다.

delete from tblBonus; -- 이 테이블이 인사 테이블과 관계가 맺어 있어서 삭제가 안되서 임의로 삭제함

delete from tblInsa where num = '1006'; -- 이제 삭제됨



--*** 오라클 사용자가 생성한 모든 식별자(테이블명, 컬럼명, 등)를 저장할 때 대문자로 저장한다.

-- 트리거 확인
select trigger_name, table_name, status from user_triggers where table_name = 'TBLINSA'; --반드시 대문자로 작성해야한다.
...
create or replace trigger trgMen
    ...
    delete
    on tblMen
    --for each row -- 행 단위 트리거 > 실제로 적용된 행의 개수를 반영
begin
    dbms_output.put_line('레코드를 삭제했습니다.' || :old.name || ',' || :old.age); -- old는 'for each row' 즉 행 단위 트리거를 작성했기 때문에 사용할 수 있는 것이다.
end trgMen;
/
...
delete from tblMen; -- 적용받는 행의 개수와 상관없이 프로시저 영역의 코드는 무조건 1번만 실행 > 문장 단위 트리거

rollback;


create or replace trigger trgMen
    before -- brfore던 after던 매개변수를 통해서 참조하기 때문에 모두 new, old를 알 수 있다.
    -- insert -- 과거가 없기 때문에 old는 사용할 수 없다.
    delete -- 현재가 없기 때문에 new를 사용할 수 없다.
    on tblMen
    for each row
begin
...
update tblMen set couple = '장도연ㅣ;ㅣ' where name = '홍길동';
...
delete from tblProject; -- 휴지통으로 지운 것
truncate table tblProject; -- 완전히 지운 것(rollback)이 안 됨
...
begin
    ...
    update tblProject set
        staff_seq = 3
            where staff_seq = :old.seq; -- 퇴사하는 직원 번혼
