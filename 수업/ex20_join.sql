...
-- 참조 방향 결정
-- : 테이블(레코드) 생성 시간을 고려 (미래가 과거를 참조 - 나중에 생성되는 테이블(레코드)가 먼저 생성되는 테이블(레코드) 참조)



drop table tblStaff; -- 자식 테이블이 참조하고 있어서 지울 수 없음 > 자식 테이블부터 없애야 함
...
-- seq를 fk로 참조하고 있기 때문에 이제 에러가 뜸
-- ORA-02291: 무결성 제약조건(HR.SYS_C008495)이 위배되었습니다- 부모 키가 없습니다
insert into tblProject (seq, project, staff_seq) values (7, '고객 유치', 5);    --5번 직원이 없음              
        
-- B. '홍길동' 퇴사
-- B.1 '홍길동' 삭제 > 에러(x) > 논리 에러
-- seq를 fk로 참조하고 있기 때문에 이제 에러가 뜸
-- ORA-02292: 무결성 제약조건(HR.SYS_C008495)이 위배되었습니다- 자식 레코드가 발견되었습니다
delete from tblStaff where seq =1; -- 홍길동이 담당하는 프로젝트를 호출하면 직원이 없음
...
from tblStaff s
    inner join tblProject p
        on s.seq = p.staff_seq; --6개
        
-- 외부조인 > 담당 프로젝트의 유무와 상관없이 모든 직원을 가져오시오. + 프로젝트 정보
select 
    *
from tblStaff s
    left outer join tblProject p
        on s.seq = p.staff_seq; --7개
...
from tblMember m
    left outer join tblRent r
        on m.seq = r.member
            where r.seq is null;  -- 한번도 대여를 안한 회원
...
select * from tblMen;       --부모(임의)
select * from tblWomen;     --자식(임의)
...
select 
    ...
    w.name as 여자
from tblMen m
    left outer join tblWomen w --솔로인 남자를 가져옴
        on m.name = w.couple;
...
select 
    ...
    w.name as 여자
from tblMen m
    right outer join tblWomen w --솔로인 여자를 가져옴
        on m.name = w.couple;
...
select 
    ...
    w.name as 여자
from tblMen m
    full outer join tblWomen w --솔로인 남자,여자를 가져옴
        on m.name = w.couple;
