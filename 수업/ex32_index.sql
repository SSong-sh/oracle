...
/*
    ...
    데이타베이스
    - 실제 DB(hdd) > 레코드 순서가 사용자가 원하는 정렬 상태가 아니다. > DBMS가 자체적으로 정렬 + 저장
    
    - 어떤 데이터를 검색 > 처음 ~ 끝까지 차례대로 검색 > table full scan
    - 특정 컬럼 선택 > 별도의 테이블애 복사 > 미리 정렬 >> 인덱스
    - 인덱스 -> 참조 > 테이블(레코드)
...
-- 인덱스 사용 검색 > 경과 시간: 00:00:00.002
select /*+ index(tblIndex idxName) */
    count(*) from tblIndex where name = '최민기';
    
    
select * from tblInsa where num = 1010;   -- 인덱스O
select * from tblInsa where name = '이순신'; --인덱스x
...
create unique index idxInsaJikwi on tblInsa(jikwi); --에러
create index idxInsaName on tblInsa(name); --고유 인덱스
...
select /*+ index(tblIndex idxHometown) */
    count(*) from tblIndex where hometown ='서울' and job ='학생'; -- 인덱스가 서울에만 걸려 있음 > 복합 인덱스를 만들어야 함
    
    
    
create index idxHomeTownJob on tblIndex(hometown, job);
select 
    count(*) from tblIndex where hometown ='서울' and job ='학생'; -- 경과 시간: 00:00:03.113
    
select /*+ index(tblIndex idxHometownJob) */
    count(*) from tblIndex where hometown ='서울' and job ='학생';  --경과 시간: 00:00:00.213
...
select /*+ index(tblIndex idxEmail) */
    count(*) from tblIndex 
    where substr(email, instr(email, '@')) = '@naver.com'; --경과 시간: 00:00:04.869

create index idxEmail on tblIndex(email); -- 연산하는 순간 다른 데이터가 됨 > index 동작 안함
...
select /*+ index(tblIndex idxEmail) */
    count(*) from tblIndex 
    where substr(email, instr(email, '@')) = '@naver.com'; -- 경과 시간: 00:00:00.507
